...
NSString * const kGRKFirebaseProviderDefaultErrorEventName = @"Error";
NSString * const kGRKFirebaseProviderDefaultPropertyKeyCategory = @"Category";
NSString * const kGRKFirebaseProviderDefaultPropertyKeySuccess = @"Success";
...
{
	if (!_errorEventName) {
		return kGRKFirebaseProviderDefaultErrorEventName;
	}
	
	return _errorEventName;
}

- (NSString *)categoryProperyName
{
	if (!_categoryProperyName) {
		return kGRKFirebaseProviderDefaultPropertyKeyCategory;
	}
	
	return _categoryProperyName;
}

- (NSString *)successProperyName
{
	if (!_successProperyName) {
		return kGRKFirebaseProviderDefaultPropertyKeySuccess;
	}
	
	return _successProperyName;
}
...
{
	if ([self.delegate respondsToSelector:@selector(providerPropertyForProperty:)]) {
		NSString *key = [self.delegate providerPropertyForProperty:property];
		if (key.length > 0) {
			property = key;
		}
	}
...
{
	if ([self.delegate respondsToSelector:@selector(providerEventForEvent:)]) {
		NSString *key = [self.delegate providerEventForEvent:event];
		if (key.length > 0) {
			event = key;
		}
	}
	...
	if (sanitizedEvent.length > 0)
	{
		...
		if (category.length > 0 && !properties[self.categoryProperyName])
		{
			GRK_GENERIC_NSMUTABLEDICTIONARY(NSString *, id) *mutableProperties = [properties mutableCopy];
			mutableProperties[self.categoryProperyName] = category;
			properties = mutableProperties;
		}

		NSDictionary *sanitizedProperties = [self sanitizeProperties:properties];
						   ...
						   properties:(nullable GRK_GENERIC_NSDICTIONARY(NSString *, id) *)properties
{
	...
	{
		GRK_GENERIC_NSMUTABLEDICTIONARY(NSString *, id) *mutableProperties = [properties mutableCopy];
		mutableProperties[self.successProperyName] = success;
		properties = mutableProperties;
	}
...
- (nullable NSDictionary *)sanitizeProperties:(nullable NSDictionary<NSString *, id> *)properties
{
